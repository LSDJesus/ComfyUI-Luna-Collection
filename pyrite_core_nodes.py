...
class Pyrite_SimpleUpscaler:
    # ... (This class remains completely unchanged from v0.0.6) ...
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
...
class Pyrite_AdvancedUpscaler:
    ...
    def upscale(self, image: torch.Tensor, upscale_model: ImageModelDescriptor, scale_by: float, resampling: str, tile_mode: str, tile_resolution: int, tile_overlap: int, supersample: bool, rescale_after_model: bool, show_preview: bool):
        device = comfy.model_management.get_torch_device()
        
        if tile_mode == 'default':
            tile_x, tile_y = 512, 512
        else:
            tile_x, tile_y = self._calculate_auto_tile_size(image.shape[3], image.shape[2], tile_resolution)
