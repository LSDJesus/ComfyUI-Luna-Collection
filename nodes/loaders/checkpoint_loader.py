...
class LunaCheckpointLoader:
    ...
    def load_checkpoint(self, ckpt_name, show_previews):
        ...
        model, clip, vae = comfy.sd.load_checkpoint_guess_config(ckpt_path, output_vae=True, output_clip=True)
        model_name = os.path.splitext(ckpt_name)[0]
        
        return (model, clip, vae, model_name)

# (Web Server Endpoint remains the same)
@PromptServer.instance.routes.get("/luna/get_checkpoint_metadata")
