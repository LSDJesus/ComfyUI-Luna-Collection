...
class ModelRegistry:
    ...
    def get_clip(self, precision: str = "bf16") -> Any:
        ...
        with self.lock:
            ...
            # Use ComfyUI's model management to ensure correct device placement
            import comfy.model_management
            
            target_device = torch.device(self.device)
            logger.info(f"[ModelRegistry] Loading CLIP on {target_device}...")
            logger.info(f"[ModelRegistry] Registry device: {self.device}")
            logger.info(f"[ModelRegistry] Target device object: {target_device}")
            
            # Store original get_torch_device function
            original_get_torch_device = comfy.model_management.get_torch_device
            
            # Temporarily patch get_torch_device to return our target device
            # (same approach as ComfyUI-MultiGPU)
            def patched_get_torch_device():
                logger.info(f"[ModelRegistry] PATCH: get_torch_device() called, returning {target_device}")
                return target_device
            
            try:
                comfy.model_management.get_torch_device = patched_get_torch_device
                ...
                # Load CLIP - will use our patched device
                # Also pass device in model_options to override ComfyUI's device selection
                self._loaded_clip = comfy.sd.load_text_encoder_state_dicts(  # type: ignore
                    state_dicts=clip_data,
                    clip_type=clip_type_enum,
                    model_options={"load_device": target_device}
                )
                
                # Explicitly move CLIP to target device (ComfyUI may override during load)
                logger.info(f"[ModelRegistry] Ensuring CLIP is on {target_device}...")
                if hasattr(self._loaded_clip, 'load_model'):
                    # Force load to target device
                    self._loaded_clip.load_device = target_device
                
                # Move patcher model
                if hasattr(self._loaded_clip, 'patcher') and hasattr(self._loaded_clip.patcher, 'model'):
                    self._loaded_clip.patcher.model = self._loaded_clip.patcher.model.to(target_device)
                    logger.info(f"[ModelRegistry] Moved patcher to {target_device}")
                
                # Move cond_stage_model
                if hasattr(self._loaded_clip, 'cond_stage_model'):
                    self._loaded_clip.cond_stage_model = self._loaded_clip.cond_stage_model.to(target_device)
                    logger.info(f"[ModelRegistry] Moved cond_stage_model to {target_device}")
                
            finally:
                # Restore original function
                comfy.model_management.get_torch_device = original_get_torch_device
