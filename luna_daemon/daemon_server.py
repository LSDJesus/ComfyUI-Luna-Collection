...
class ModelRegistry:
    ...
    def model_forward(self, x: torch.Tensor, timesteps: torch.Tensor,
        ...
        # Nest contexts: FB cache (outer) -> LoRA (inner) -> inference
        # This ensures FB cache sees LoRA-modified model
        
        # Transform kwargs from ComfyUI's apply_model format to diffusion_model format
        # ComfyUI sends: c_crossattn, y, control, transformer_options, etc.
        # diffusion_model expects: context, y, control, transformer_options, etc.
        diffusion_kwargs = {}
        
        # Map c_crossattn -> context (but we already have context as a param)
        # So just pass through valid diffusion model parameters
        for key, value in kwargs.items():
            if key == 'c_crossattn':
                # This should be 'context' for diffusion_model
                # But we already extract context separately, so skip if context is provided
                if context is None and value is not None:
                    context = value.to(device)
            elif key in ('y', 'control', 'transformer_options', 'num_video_frames', 
                        'image_only_indicator', 'time_context'):
                # These are valid diffusion_model parameters
                if isinstance(value, torch.Tensor):
                    diffusion_kwargs[key] = value.to(device)
                else:
                    diffusion_kwargs[key] = value
            # Skip other keys like c_concat, crossattn_controlnet, etc.
            # Those are handled at a higher level in ComfyUI's _apply_model
        
        with apply_fb_cache_transient(model, fb_config):
            if lora_stack:
                with self._apply_lora_transient(model, lora_stack):
                    with torch.inference_mode():
                        result = model.model.diffusion_model(x, timesteps, context=context, **diffusion_kwargs)
            else:
